  ...
  <div class="content-header el-custom linkList">
    ...
    <el-dialog title="Detail" :visible.sync="dialogTableVisible">
      <el-table :data="dataList" :span-method="objectSpanMethod" header-row-class-name="table-header" stripe border style="width: 100%;font-size:12px;">
        <el-table-column align="center" width="90" label="Catetory" prop="parent_en">
        </el-table-column>
        <el-table-column align="center" width="80" label="Method" prop="name_en">
        </el-table-column>
        <el-table-column align="center" label="Min Bet" prop="min_bet">
          <template slot-scope="scope">
            <el-row>
              <el-input style="width:60px;" @blur="handleMinBet(scope.row, $event)" v-model="scope.row.copy_min_bet" :max="scope.row.min_bet"></el-input> &lt;= {{scope.row.min_bet}}
            </el-row>
          </template>
        </el-table-column>
        <el-table-column align="center" label="Max Bet" prop="max_bet">
          <template slot-scope="scope">
            <el-row>
              <el-input style="width:60px;" @blur="handleMaxBet(scope.row, $event)" v-model="scope.row.copy_max_bet" :max="scope.row.max_bet"></el-input> &lt;= {{scope.row.max_bet}}
            </el-row>
          </template>
        </el-table-column>
        <el-table-column align="center" label="Reward" prop="prize">
          <template slot-scope="scope">
            <el-row>
              ...
              <el-input :class="scope.row.showRuler?'error-ruler':''" style="width:60px;" @blur="handlePrize(scope.row, $event)" v-model="scope.row.copy_prize" :max="scope.row.prize"></el-input> &lt;= {{scope.row.prize}}
            </el-row>
          </template>
        </el-table-column>
        <el-table-column align="center" label="Discount" prop="discount">
          <template slot-scope="scope">
            <el-row>
              <p v-if="scope.row.showRuler" style="color:#c83a4c;">ILLEGAL</p>
              <el-input :class="scope.row.showRuler?'error-ruler':''" style="width:60px;" @blur="handleDiscount(scope.row, $event)" v-model="scope.row.copy_discount"></el-input> &lt;= {{scope.row.discount}}
            </el-row>
          </template>
        </el-table-column>
        <el-table-column align="center" label="Stock" prop="stock">
          <template slot-scope="scope">
            <el-row>
              <el-input style="width:60px;" @blur="handleStock(scope.row, $event)" v-model="scope.row.copy_stock"></el-input> &lt;= {{scope.row.stock}}
            </el-row>
          </template>
        </el-table-column>
      </el-table>
    </el-dialog>
    <el-dialog title="Edit" :center="false" :visible.sync="detailVisible">
      <div class="body-middle">
        <el-form>
          <el-form-item label="Credit：">
            <el-input v-model.trim="cash_credit" >
            </el-input>
          </el-form-item>
        </el-form>
      </div>
      <div slot="footer" class="dialog-footer">
        <el-button type="primary" @click="updateLink" class="yes">Confirm</el-button>
      </div>
    </el-dialog>
  </div>
</template>
<script>
  ...
  export default {
    ...
    created() {
      ...
      let data = this.$store.state.formatMenuFn(this.index1, this.index2, 'linkList');
      ...
      getDataList() {
        ...
        request.http(
          ...
          '/agency/link/list',
          {
            ...
            superiorname: vm.loginname || 'KINGA',
            type: [0, 1]
          },
          success => {
            let code = success.returncode;
