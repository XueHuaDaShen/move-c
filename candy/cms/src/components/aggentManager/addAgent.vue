        ...
        <table class="search-table">
          <tbody>
            ...
            <tr>
              ...
              <td><el-input v-model.trim="loginname" placeholder="username" clearable style="width:140px;"></el-input></td>
            </tr>
            <tr>
              <td><label>Password：</label></td>
              <td><el-input type="password" v-model.trim="pwd" placeholder="password" clearable style="width:140px;"></el-input></td>
            </tr>
            <tr>
              <td><label>Confirm Password：</label></td>
              <td><el-input type="password" v-model.trim="confirm_pwd" placeholder="confirm password" clearable style="width:140px;"></el-input></td>
            </tr>
            <!-- <tr>
              <td><label>Email.：</label></td>
              <td><el-input v-model.trim="email" placeholder="email." clearable style="width:140px;"></el-input></td>
            </tr>
            <tr>
              <td><label>Phone NO.：</label></td>
              <td><el-input v-model.trim="phone" placeholder="phone no." clearable style="width:140px;"></el-input></td>
            </tr>
            <tr>
              <td><label>First Name：</label></td>
              <td><el-input v-model.trim="firstname" placeholder="first name" clearable style="width:140px;"></el-input></td>
            </tr>
            <tr>
              <td><label>Last Name：</label></td>
              <td><el-input v-model.trim="lastname" placeholder="last name" clearable style="width:140px;"></el-input></td>
            </tr> -->
            <!-- <tr>
              <td><label>Credit：</label></td>
              <td><el-input type="text" v-model.trim="cash_credit" @blur="handleCredit($event)" placeholder="credit" clearable style="width:140px;"></el-input></td>
            </tr> -->
            <tr>
...
<script>
  ...
  export default {
    ...
    created() {
      ...
      let data = this.$store.state.formatMenuFn(this.index1, this.index2, 'addAgent');
    ...
    methods: {
      ...
      prizeRulerFn(row) {
        ...
        return !(Number(prize*100/motherprize) + Number(discount)  < 100);
      },
      getDataList() {
        const vm = this;
        let list = [];
      ...
      createLink() {
        ...
        // console.log(user_refund)
        // return false;
        // if(!vm.user){
        //   vm.message('请选择用户类型', 'error');
        //   return false;
        // }
        if(!vm.kingName){
        ...
        if(!vm.loginname){
          vm.message('please inut user name', 'error');
          return false;
        }
        if (!validator.regexpInput(vm.loginname)) {
          vm.message("User names require 3-16 bits of letters or numbers", "error");
          return;
        }
        if(!vm.pwd){
          vm.message('please input password', 'error');
          return false;
        }
        if (!validator.regexpPsd(vm.pwd)) {
          vm.message("A combination of letters and numbers with a password of more than 6 digits", "error");
          return;
        }
        if(!vm.confirm_pwd){
        ...
        if(vm.confirm_pwd !== vm.pwd){
          vm.message('The two passwords don\'t match', 'error');
          return false;
        }
        // if(!vm.email){
        //   vm.message('please input email', 'error');
        //   return false;
        // }
        // if (!validator.email(vm.email)) {
        //   vm.message("Illegal mailbox", "error");
        //   return;
        // }
        // if(!vm.phone){
        //   vm.message('please input phone', 'error');
        //   return false;
        // }
        // if(!vm.firstname){
        //   vm.message('please input firstname', 'error');
        //   return false;
        // }
        // if(!vm.lastname){
        //   vm.message('please input lastname', 'error');
        //   return false;
        // }
        // if(!vm.cash_credit){
        //   vm.message('please input credit', 'error');
        //   return false;
        // }
        if(!vm.templateid){
