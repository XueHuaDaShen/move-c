      ...
      >
        <el-table-column align="center" label="User Name" width="126">
          <template slot-scope="scope">
            ...
            <el-button type="text" @click="getUserChildren(scope.row)">{{scope.row.loginname}}</el-button>
          </template>
        </el-table-column>
        ...
        <el-table-column align="center" prop="create_at" :formatter="formatTime" label="Register time" width="130px"></el-table-column>
        <el-table-column align="center" fixed="right" label="Action" width="284px">
          <template slot-scope="scope">
            <el-row class="tr">
              ...
              <el-button size="mini" @click="enableChange(scope.row,0)" class="small yes" v-if="scope.row.status == 1">Enable</el-button>
              <el-button size="mini" @click="enableChange(scope.row,1)" class="small yes" v-if="scope.row.status == 0">Disable</el-button>
            </el-row>
            <el-row class="tr">
              <el-button size="mini" @click="getUserInfoFn(scope.row)" class="small mt-10 edit">User Info</el-button>
...
export default {
  ...
  created() {
    ...
    let data = this.$store.state.formatMenuFn(this.index1, this.index2, 'userlistTree');
  ...
  methods: {
    ...
    getUserChildren(row) {
      this.searchname = row.loginname;
      if(this.breadcrumbArr.indexOf(this.searchname) > -1){
        return false;
      }
      this.breadcrumbArr.push(this.searchname)
      this.getUserList();
    },
    getSummaries(param) {
      const {
