  ...
  <div class="rechargeLot-wrap el-custom">
    ...
    <div class="data-table" v-loading="loading">
      <el-table :data="monthProfitListData" header-row-class-name="table-header" stripe border style="width: 100%;font-size:12px;"
        @selection-change="handleSelectionChange" max-height="450" :show-summary="true" sum-text="Total" :summary-method="getSummaries">
        ...
        <el-table-column align="center" prop="team_commission" label="团队分红总额">
        </el-table-column>
        <el-table-column align="center" prop="from_day" :formatter="formatFenhong" label="开始时间">
        </el-table-column>
        <el-table-column align="center" prop="to_day" :formatter="formatFenhong" label="结束时间">
        </el-table-column>
        <el-table-column align="center" prop="day" :formatter="formatTime" label="应发分红日期">
        </el-table-column>
        <el-table-column align="center" prop="pay_at" :formatter="formatTime" label="实发分红日期">
        </el-table-column>
      ...
      <div class="body-middle">
        <div class="form">
          <div class="form-row">
            ...
            <el-date-picker format="dd-MM-yyyy HH:mm:ss" v-model="onekeyForm.time" align="right" type="date" placeholder="选择日期">
            </el-date-picker>
          </div>
        </div>
      </div>
...
<script>
  ...
  export default {
    ...
    created() {
      ...
      let data = this.$store.state.formatMenuFn(this.index1, this.index2, 'monthProfit');
    ...
    methods: {
      ...
      formatTime(row, column, cellValue) {
        if (cellValue) {
          return moment(cellValue).utcOffset(8).format("DD-MM-YYYY");
        }
        return '--'
      },
      ...
      getMonthProfitList() {
        ...
        request.http(
          ...
          success => {
            vm.loading = false;
            // console.log("monthProfitList--------", success);
            let code = success.returncode;
            if (code === 200) {
              vm.monthProfitListData = success.data;
