<template>
  <div class="common team-recharge-wrap">
    <div class="container">
      <div class="tab">
        <router-link class="tab-item" :to="{name:'teamBet'}">
          <div class="item-inner">{{$t('message.team_bet')}}</div>
        </router-link>
        <router-link class="tab-item selected" :to="{name:'teamRecharge'}">
            ...
            <table class="record-table mt-60">
              ...
              <tbody>
                <tr v-if="!noResult" v-for="(item,index) in list" :key="index">
                  ...
                  <td>
                    <div class="cell">{{item.loginid ? item.loginid.firstname :'--'}}</div>
                  </td>
                  <td>
...
<script>
  ...
  export default {
    ...
    computed: {},
    created() {
      sessionStorage.setItem("th_tag_url", "teamRecharge");
    },
    mounted() {
      this.setTimeToday();
      this.handleSearch();
    ...
    methods: {
      ...
      onSubmit() {
        ...
        let url = '/agency/team/recharge/list' + '?page_size=' + vm.pageSize + '&page_num=' + vm.pageIndex +
          ...
          new Date(vm.endTime).toISOString() + '&status=2';
