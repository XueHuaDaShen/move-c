<template>
  <div class="lottery-detail">
    ...
    <div class="alert-confirm-bets-wrap" :style="{left: confirmLeft + 'px', top: confirmTop + 'px'}" v-if="showConfirm">
    ...
    <div class="alert-confirm-bets-wrap" style="min-height:0" :style="{left: confirmLeft + 'px', top: confirmTop + 'px'}"
      v-if="showSuccess">
  ...
  methods: {
    ...
    selectedNumFn(val, isRowCol=false, isRun=false, selected = false) {
      ...
      for(let i in selectedPlay){
        ...
        for(let j in shoppingCart){
          ...
          if(shop.play_id === v.play_id){
            isFind = true;
            nums = shop.nums;
          }
        }
        if(!isFind){ // 购彩篮中 没有当前的玩法
          let obj = {
          ...
          if (index > -1) { // 当前号码已存在
            let elseV; // 另一个玩法 （玩法只有两个）
            if(i == 0){
              elseV = selectedPlay.length > 1 ? selectedPlay[1] : false;
            }else{
              elseV = selectedPlay[0];
            }
            // debugger
            if(!elseV){
              if(full){
                // debugger
                this.isAllInArr(nums, val, isRowCol, isRun);
              }else{
                // debugger
                if(isRowCol){
                  // debugger
              ...
              let elseIndex = -1;
              let elseNums = [];
              let isFindElseVId = false;
              shoppingCart.filter(s => {
                // debugger
                if(s.play_id === elseV.play_id){
                  // debugger
                  elseNums = s.nums;
                  isFindElseVId = true;
                }
              ...
              if(elseIndex > -1){
                if(full){
                  // debugger
                  this.isAllInArr(nums, val, isRowCol, isRun);
                  this.isAllInArr(elseNums, val, isRowCol, isRun);
                  break;
                }else{
                  // debugger
                  if(isRowCol){
                ...
                if(full && !isRowCol){
                  ...
                  numArr.filter(n => {
                    ...
                    elseNums.push({
                      num: n,
                      money: elseV.one_min_money,
                      win: elseV.prize,
                      discount: elseV.discount,
                      number: pushLen
                    });
                  })
                  // debugger
                  if(!isFindElseVId){
                    ...
                    let obj = {
                      kind_id: this.kind_id,
                      kind_name: this.kind_name,
                      play_id: elseV.play_id,
                      play_name: elseV.play_name,
                      one_min_money: elseV.one_min_money,
                      one_max_money: elseV.one_max_money,
                      one_max_bets: elseV.one_max_bets,
                      prize: elseV.prize,
                      discount: elseV.discount,
                      nums: elseNums
                    }
    ...
    setAlertSize() {
      ...
      try {
        ...
        let top = (window.innerHeight - el.offsetHeight) /
          2
        this.confirmLeft = left < 0 ? 0 : left;
        this.confirmTop = top < 0 ? 0 : top;
      } catch (error) {}
    ...
    confirmBets() {
      ...
      this.$http.post('/lottery/issue/vote', paramCryptFn(data), {
        ...
        if (code == 200) {
          ...
          this.$nextTick(() => {
            // this.consoleFn(window.innerWidth, window.innerHeight, document.getElementsByClassName('alert-confirm-bets-wrap'))
            // let left = window.innerWidth / 2 - 520 / 2;
            // let top = (window.innerHeight - document.getElementsByClassName('alert-confirm-bets-wrap')[0].offsetHeight) /
            //   2
            // this.confirmLeft = left < 0 ? 0 : left;
            // this.confirmTop = top < 0 ? 0 : top;
            this.setAlertSize();
