...
class Live extends React.Component {
    ...
    exportFn() {
        ...
        this.state.table = TableExport(document.getElementsByTagName("table"), {formats: ['xls'], ignoreCols: [3], exportButtons: false});
        ...
        if( flag === '' ){
            Object.keys(data).forEach(function (key) {
                source.push({
                    key: data[key].pid,
                    ID: data[key].pid,
                    LiveDays: data[key].liveDays,
                    LiveLength: data[key].liveLength,
                    Method: data[key].pid
                })
            });
        }else if( flag == 'detail' ){
            ...
            Object.keys(data).forEach(function (key) {
                ...
                source.push({
                    key: k,
                    Date: moment(data[key].create_at).format('Y/M/D H:M:S'),
                    BeginAt: moment(data[key].create_at).format('Y/M/D H:M:S'),
                    Duration: Math.ceil(data[key].interval_val/60)+'  (Minute)'
                })
            });
        }
            ...
            <div className="rightView">
                <div className="salary-head">
                    <div className="content">
                        {!state.isDetail?<Button size="large" style={{marginRight:5}} type="primary" onClick={this.exportFn}>Export</Button>:''}
                        { !state.isDetail ?
