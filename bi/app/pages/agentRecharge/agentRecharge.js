...
class AgentRecharge extends React.Component {
    constructor() {
        super();
        this.state = {
    ...
    List() {
        ...
        Axios2(
            'post',
            '/finance/recharge/recharge_agency',
            {
                begintime: this.state.startDate,
                endtime: this.state.endDate,
                typeOfDate: this.state.type,
                rechargeAgency: this.state.rechargeAgency
            },
            {
    ...
    render() {
        ...
        let options = this.state.agencyList.map(item => <Option key={item._id}>{item.name}</Option>);

        return (
            <div className="rightView">
                <div className="salary-head">
                    <div className="content">
                        <Select size="large" placeholder="Select Agent" style={{width: 120, marginRight: 5}}
                                onSelect={this.onChangeDataType}>
...
export default connect(mapStateToProps, mapDispatchToProps)(AgentRecharge);
