<template>
  <div class="channel_2" v-loading="loading" element-loading-background="rgba(0, 0, 0, 0.5)">
    <div class="channel_2-inner">
      <div class="fill-amount">
        <div class="amount-row">
          <div class="flex-col">
            <div class="mt-20 label">{{$t('message.bank_for_deposit_withdrawal')}} <span class="red" v-if="form.bankid.require">*</span></div>
        ...
        <div class="amount-row">
          <div class="flex-col">
            <div class="mt-20">{{$t('message.amount')}} <span class="red" v-if="form.amount.require">*</span></div>
            <div class="mt-10 input-outer">
        ...
        <div class="mt-60 amount-row mb-60">
          <button class="user-btn" @click="onSubmit()" :disabled='isClick'>{{$t('message.next')}}</button>
        </div>
      </div>
    </div>
  </div>
    ...
    methods: {
      ...
      getDepositToken(data) {
        ...
        this.$http.post('/paysec/payin',paramCryptFn({
          ...
          if(code == 200){
            this.sendDepositToken(res.body.data.token)
              ...
              for (let key in data) {
                ...
                if (obj.enabled === 1) {
                  ...
                  let imgObj = vm.imgArr.find(item => {
                    ...
                    return item.key === obj.name.split(' ')[0];
                  });
                  obj.img = imgObj.val;
