    ...
    <div class="user-table mt-60" v-loading="loading" element-loading-background="rgba(0, 0, 0, 0.5)">
      <!-- <el-table
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            {{scope.row.apply_amount | filterCash}}
          </template>
        </el-table-column>
        <el-table-column
          :label="$t('message.bonus')">
          <template slot-scope="scope">
            {{scope.row.packageid && scope.row.packageid.free | filterCash}}
          </template>
        </el-table-column>
        <el-table-column
      ...
      <!-- <table>
        <thead>
          <tr>
            ...
            <th :style="{width: Math.floor(100/5)+'%'}">
              <div class="cell">{{$t('message.remark')}}</div>
            </th>
          </tr>
        </thead>
        <tbody v-if="!dataNull">
