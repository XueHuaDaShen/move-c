  ...
  <div class="rechargeLot-wrap el-custom">
    ...
    <div class="data-table" v-loading="loading">
      <el-table :data="rechargeListData" header-row-class-name="table-header" stripe border style="width: 100%;font-size:12px;"
        ...
        <el-table-column align="center" prop="create_at" :formatter="formatTime2" label="Submit Time">
        </el-table-column>
        ...
        <el-table-column align="center" prop="status" fixed="right" label="Detail" width="150">
          <template slot-scope="scope">
            <el-button v-if="scope.row.status==1 || scope.row.status==4" @click="toRechargeFn(scope.row)" class="yes small">Audit</el-button>
    ...
    <el-dialog title="Audit" :center="false" :visible.sync="dialogFormVisible">
      <div class="body-middle">
        <el-form ref="form" :rules="rules" :model="form">
          <el-form-item label="User Name：" v-if="form.username">
            <el-input v-model.trim="form.username" disabled>
            </el-input>
          </el-form-item>
          <el-form-item label="First Name：" v-if="form.firstname">
            <el-input v-model.trim="form.firstname" disabled>
            </el-input>
          </el-form-item>
          <el-form-item label="Last Name：" v-if="form.lastname">
            <el-input v-model.trim="form.lastname" disabled>
            </el-input>
          </el-form-item>
          ...
          <el-form-item label="Amount：" prop="realRecharge">
            <el-input v-model.trim="form.realRecharge" placeholder="please input amount" disabled>
            </el-input>
          </el-form-item>
      ...
      <div slot="footer" class="dialog-footer" style="text-align:center;">
        <el-button type="primary" @click="submitForm('form')" class="yes" :loading="rechargeLoading">Confirm</el-button>
...
export default {
  ...
  created() {
    ...
    let data = this.$store.state.formatMenuFn(this.index1, this.index2, 'adminDeposit');
  ...
  methods: {
    ...
    toRechargeFn(row) {
      ...
      this.currId = row._id;
      // this.currentCash = row.amount;
      this.recharge = row.amount;
      this.form.realRecharge = row.amount;
      this.form.bank_no = row.account ? row.account.card_no : "";
    ...
    getDetail(row) {
      ...
      this.detail.bank_no = row.account ? row.account.card_no : "";
      this.detail.remark = row.remark;
      this.detail.username = row.loginname;
      this.detail.firstname = row.account.firstname;
      this.detail.lastname = row.account.lastname;
    },
    closeDetail() {
