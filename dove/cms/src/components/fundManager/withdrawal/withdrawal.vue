  ...
  <div class="rechargeLot-wrap el-custom">
    ...
    <div class="data-table" v-loading="loading">
      <el-table :data="withdrawListData" header-row-class-name="table-header" stripe border style="width: 100%;font-size:12px;"
        ...
        <el-table-column align="center" prop="accountid.card_no" width="150" label="Bank Account">
        </el-table-column>
        ...
        <el-table-column align="center" prop="status" fixed="right" label="Detail" width="150">
          <template slot-scope="scope">
            <div class="flex-row">
              <el-button v-if="scope.row.status==1" @click="toWithdrawFn(scope.row)" class="small yes">Audit</el-button>
      ...
      <div class="body-middle">
        ...
        <table class="dialog-table" style="margin-top:30px;">
          ...
          <tr>
            ...
            <td align="left" class="td-right">
              {{formatTime2(detailForm.update_at)}}
            </td>
          </tr>
        </table>
      </div>
...
export default {
  ...
  created() {
    ...
    let data = this.$store.state.formatMenuFn(this.index1, this.index2, 'withdrawal');
  ...
  methods: {
    ...
    submitForm(formName) {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          let url = '/trade/withdrawal/update'
    ...
    toWithdrawFn(row) {
      ...
      this.form.accountName = row.accountid.firstname + row.accountid.lastname;
      this.form.firstname = row.accountid.firstname;
      this.form.lastname = row.accountid.lastname;
      this.form.account = row.loginname;
      this.form.bank = row.accountid.bank;
      this.form.card_no = row.accountid.card_no;
      this.form.cash_apply = row.amount;
    ...
    getWithdrawLotList() {
      ...
      request.http(
        ...
        '/trade/withdrawal/list', {
          ...
          amount_start: vm.outcomeRange.min,
          amount_end: vm.outcomeRange.max
        },
        (success) => {
          vm.loading = false;
