...
export default {
  ...
  created() {
    ...
    let data = this.$store.state.formatMenuFn(this.index1, this.index2, 'depositRetention');
  ...
  methods: {
    ...
    formatData(data) {
      ...
      this.dateData = [];
      let coll = 30;
      this.dateLength = data[0].num_deposit_remain_user.length;
      for(let i in data){
        let t = time + day*(i+1);
        this.dateTime.push({day: data[i].day, value: data[i].num_deposit_first || 0});
        let arr = [];
        let end = data[0].num_deposit_remain_user.length
        for(let j=0; j<end; j++){
          let remain = data[i].num_deposit_remain_user;
          let len = remain.length;
          if(j < len){
            arr.push(remain[j]&&data[i].num_deposit_first ? Math.round((remain[j]/data[i].num_deposit_first)*100) : 0)
          }else{
            arr.push('none')
          }
        }
