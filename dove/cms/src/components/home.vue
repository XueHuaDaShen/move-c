...
<script>
  ...
  export default {
    ...
    data() {
      return {
        colors: ['#FF9291', '#51E6C2', '#FBB91B', '#E15454', '#5BD4FF', '#B831FF', '#E98DF3', '#8EB4FF'],
        activeIndex: '/', //默认路由
    ...
    created() {
      const vm = this;
      let isLogin = localStorage.getItem('dove-cms-isLogin') === 'true';
      if(!isLogin){
        this.$router.push({
          name: 'login'
        })
      ...
      this.getCurrentTime();
      this.timeInterval();
      this.userName = localStorage.getItem('dove-cms-loginname');
      this.menuData = JSON.parse(localStorage.getItem('dove-cms-menus'));
    },
    methods: {
      timeInterval() {
        const vm = this;
        setInterval(vm.getCurrentTime, 1000)
        setInterval(vm.getCount, 1000 * 60 * 1)
      },
      getCurrentTime() {
