<template>
  <div class="admin-wrap el-custom">
      ...
      <div class="search-wrap">
        ...
        <div class="search-content">
          <div class="search-inner-wrap">
            ...
            <el-input clearable v-model.trim="adminName" placeholder="user name" style="width:114px;"></el-input>
          </div>
          <div class="search-inner-wrap">
            <label>Group：</label>
            <el-select clearable v-model="group.value" placeholder="please select" class="small" @focus="groupFocus()" :loading="group.loading"
          ...
          <div class="search-inner-wrap">
            <label>Status：</label>
            <el-select clearable v-model="status.value" placeholder="please select" class="small">
    ...
    <div class="content-middle">
      <div class="search-wrap">
        <div class="search-inner-wrap">
          <el-button type="success" @click="createAdmin" size="medium" class="edit">Create CMS User</el-button>
        </div>
      </div>
    </div>
    <div class="data-table" v-loading="loading">
      <el-table :data="activityLogListData" header-row-class-name="table-header" align="left" stripe border style="width: 100%;font-size:12px;"
        max-height="500">
        <el-table-column width="180" prop="loginname" label="User Name">
        </el-table-column>
        <el-table-column width="180" label="Group">
          <template slot-scope="scope">
            {{formatRole(scope.row)}}
          </template>
        </el-table-column>
        <el-table-column prop="create_at" :formatter="formatTime" width="180" label="Create At">
        </el-table-column>
        <el-table-column :formatter="formatTime" width="180" label="Last Login">
          <template slot-scope="scope">
        ...
        <el-table-column prop="message" label="Remark">
        </el-table-column>
        <el-table-column width="240" label="Action">
          <template slot-scope="scope">
            <el-row class="tr">
              <el-button @click="editPwd(scope.row)" class="small edit">Modify Password</el-button>
              <!-- <el-button @click="operateHistory(scope.row)" class="small no">操作记录</el-button> -->
            </el-row>
            <el-row class="tr">
              <el-button @click="updateRole(scope.row)" class="small edit mt-10">Modify Group</el-button>
              <!-- <el-button @click="operateHistory(scope.row)" class="small no">操作记录</el-button> -->
            </el-row>
          </template>
        </el-table-column>
    ...
    <el-dialog title="Create CMS User" :visible.sync="dialogFormVisible" @close="resetForm('form')">
      <div class="body-middle">
        <div class="form">
          ...
          <div class="form-row">
            <span class="exp">Password</span>
            <span class="exp-after">:</span>
            <el-input type="password" v-model.trim="form.pw" clearable></el-input>
          </div>
          <div class="form-row">
            <span class="exp">Group</span>
            <span class="exp-after">:</span>
            <el-select clearable v-model="form.willCheck" placeholder="please select" class="small">
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="createAdminFn('form')" class="yes" :loading="createLoading">Confirm</el-button>
      </div>
    </el-dialog>
    <el-dialog title="Modify Group" :visible.sync="dialogFormVisible2" @close="resetForm('updateRoleForm')">
      <div class="body-middle">
        <div class="form">
          <div class="form-row">
            <span class="exp">User Name</span>
          ...
          <div class="form-row">
            <span class="exp">Old Group</span>
            <span class="exp-after">:</span>
            <el-input type="text" v-model.trim="updateRoleForm.checked" disabled></el-input>
          </div>
          <div class="form-row">
            <span class="exp">New Group</span>
            <span class="exp-after">:</span>
            <el-select clearable v-model="updateRoleForm.willCheck" placeholder="select" class="small">
              <el-option v-for="item in roleList" :key="item._id" :label="item.name" :value="item._id">
              </el-option>
            </el-select>
          </div>
      ...
      <div slot="footer" class="dialog-footer">
        <el-button @click="resetForm('updateRoleForm')" class="no">Cancel</el-button>
        <el-button type="primary" @click="updateUserRole('updateRoleForm')" class="yes" :loading="groupLoading">Confirm</el-button>
      </div>
    </el-dialog>
    <el-dialog title="操作记录" :visible.sync="dialogFormVisible3">
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="dialogFormVisible3 = false">Confirm</el-button>
      </div>
    </el-dialog>
    <el-dialog title="Modify Password" :visible.sync="dialogFormVisible4" @close="resetForm('pwdForm')">
      <div class="body-middle">
        <div class="form">
          <div class="form-row">
            <span class="exp">User Name</span>
          ...
          <div class="form-row">
            <span class="exp">New Password</span>
            <span class="exp-after">:</span>
            <el-input type="password" v-model.trim="pwdForm.newpw" clearable></el-input>
          </div>
          <div class="form-row">
            <span class="exp">Confirm Password</span>
            <span class="exp-after">:</span>
      ...
      <div slot="footer" class="dialog-footer">
        <el-button @click="resetForm('pwdForm')" class="no">Cancel</el-button>
        <el-button type="primary" @click="editPwdFn('pwdForm')" class="yes" :loading="psdLoading">Confirm</el-button>
      </div>
    </el-dialog>
  </div>
</template>
<script>
  ...
  export default {
    ...
    data() {
      var validatePass2 = (rule, value, callback) => {
        if (value === '') {
          ...
          callback(new Error('Two input password does not match!'));
        } else {
          callback();
        }
      };
    ...
    created() {
      ...
      let data = this.$store.state.formatMenuFn(this.index1, this.index2, 'adminList');
    ...
    methods: {
      ...
      formatTime2(row) {
        if (row.log && row.log.create_at) {
          return moment(row.log.create_at).format("DD-MM-YYYY HH:mm:ss");
        }
        return "--"
      },
      ...
      editPwdFn(formName) {
        ...
        if (!regexpPsd(vm.pwdForm.newpw)) {
          this.$message({
            message: 'Incorrect password format for new password',
            type: 'error',
            duration: vm.duration,
            center: true
          })
        ...
        if (!regexpPsd(vm.pwdForm.checkPass)) {
          this.$message({
            message: 'Incorrect password format for confirmed password',
            type: 'error',
            duration: vm.duration,
            center: true
          })
        ...
        if (vm.pwdForm.newpw != vm.pwdForm.checkPass) {
          this.$message({
            message: 'Two input password does not match!',
            type: 'error',
            duration: vm.duration,
            center: true
          })
      ...
      createAdminFn(formName) {
        ...
        if (!email(vm[formName].loginname)) {
          this.$message({
            message: 'User name must be mailbox.',
            type: 'error',
            duration: vm.duration,
            center: true
          })
