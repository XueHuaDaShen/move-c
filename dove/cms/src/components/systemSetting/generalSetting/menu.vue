<template>
  <div class="el-custom system-wrap">
    ...
    <div class="content-middle">
      <div class="search-wrap">
        <div class="search-inner-wrap">
          <el-button type="success" @click="createMenu('一', '')" size="medium" class="edit">Add Root Menu</el-button>
        </div>
      </div>
    </div>
    <div class="data-table" v-loading="loading">
      <el-table :data="activityLogListData" header-row-class-name="table-header" fixed align="left" stripe border style="width: 100%;font-size:12px;">
        <el-table-column width="200" label="Root Menu" prop="menu_name">
          <template slot-scope="scope">
            ...
            <el-button class="small my-add-button" @click="createMenu('二', scope.row._id)" icon="el-icon-plus" type="success"
              circle></el-button>
            <!-- <el-button @click="editMenu(scope.row)" class="small edit">Edit</el-button> -->
          </template>
        </el-table-column>
        <el-table-column align="center" width="80" label="Action" prop="order">
          <template slot-scope="scope">
            <el-button @click="editMenu('一', scope.row)" class="small edit" style="padding:0 12px;">Edit</el-button>
          </template>
        </el-table-column>
        <el-table-column label="Child Menu">
          <template slot-scope="props">
            <el-form label-position="left" inline class="demo-table-expand">
              ...
              <el-table :data="props.row.child" :show-header="false" :default-expand-all="true">
                ...
                <el-table-column align="center" width="80" label="Sequence" prop="order">
                  <template slot-scope="scope">
                    <el-button @click="editMenu('二', props.row, scope)" class="small edit" style="padding:0 12px;">Edit</el-button>
                  </template>
                </el-table-column>
                    ...
                    <el-form label-position="left" inline class="demo-table-expand">
                      <el-table :data="props.row.child" :show-header="false">
                        ...
                        <el-table-column width="180" align="center">
                          <template slot-scope="scope">
                            <el-button @click="editMenu('三', props.row, scope)" class="small edit" style="padding:0 12px;">Edit</el-button>
                          </template>
                        </el-table-column>
                      </el-table>
                    </el-form>
    ...
    <el-dialog :title="operateTitle" :visible.sync="dialogFormVisible" @close="resetForm('form')">
      <div class="body-middle">
        <div class="form">
          <el-form :model="form" :rules="rules" ref="form">
            <el-form-item :label="(menu_type+'级菜单') === '一级菜单'? 'Root Menu' : ((menu_type+'级菜单') === '二级菜单'?'Child Menu':'Grand Child Menu')" prop="menu_name">
              <el-input v-model="form.menu_name" auto-complete="off"></el-input>
            </el-form-item>
            <el-form-item label="Sequence" prop="order">
              <el-input v-model.number="form.order" @blur="()=>{!isNaN(form.order)&&(form.order = Math.floor(form.order))}"
                auto-complete="off"></el-input>
            </el-form-item>
            <el-form-item label="Router" prop="url" v-if="menu_type === '三'">
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button v-if="isCreate" type="primary" @click="createMenuFn('form')" class="yes" :loading="createLoading">Confirm</el-button>
        <el-button v-if="!isCreate" type="primary" @click="editMenuFn('form')" class="yes" :loading="updateLoading">Confirm</el-button>
      </div>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        order: [{
          required: true,
          message: 'Sequence value must be positive integers',
          type: 'number',
          trigger: 'blur'
        }],
        url: [{
  ...
  created() {
    ...
    let data = this.$store.state.formatMenuFn(this.index1, this.index2, 'menu');
...
<style>
  ...
  .system-wrap .el-table__body-wrapper{
