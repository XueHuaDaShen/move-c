<template>
  <div class="el-custom announcement">
    ...
    <el-dialog :title="formTitle" :before-close="closeForm" :visible.sync="formVisible" class="relative-dialog">
      <div class="body-middle">
        <div class="form">
          ...
          <div class="form-row">
            <span class="exp">Duration time</span>
            <span class="exp-after">:</span>
...
export default {
  ...
  created() {
    ...
    let data = this.$store.state.formatMenuFn(this.index1, this.index2, 'announcement');
  ...
  methods: {
    ...
    editActivity(item) {
      ...
      if (!this.form.order) {
        this.message('Please enter a sort value', 'error')
        return;
      }
      if (!this.forever) {
        if (!this.form.time.toString()) {
          this.message('Please select time', 'error')
          return;
        }
      }
      ...
      let isFile1 = false, content_true = false;
      if (isFile1) {
        if (!content_true) {
          request.upload(
            "post",
