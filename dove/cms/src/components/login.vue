    ...
    <el-col :span="6" class="login" v-loading="loading" :style="login">
      <div class="grid-content bg-purple">
        ...
        <el-form :model="loginForm" ref="loginForm" class="demo-dynamic" style="position:relative;">
          ...
          <el-form-item>
            <el-button @click="submitForm('loginForm')" :disabled="isClick" class="login-btn" plain>Login</el-button>
          </el-form-item>
  ...
  export default {
    ...
    methods: {
                  ...
                  request.login(
                    ...
                    success => {
                      ...
                      if (code == "304") {
                        ...
                        vm.loginStatus = "The user does not exist";
                        setTimeout(function () {
                        ...
                        vm.isLoginError = true;
                        vm.loginStatus = "Password mistake";
                        setTimeout(function () {
                          vm.isLoginError = false;
                          vm.loginStatus = "";
                        }, 2000);
                      } else if (code == "200") {
                        localStorage.setItem("dove-cms-token", success.data.token);
                        localStorage.setItem("dove-cms-loginname", vm.loginForm.username);
                        // vm.$router.push({
                        //   path: "/home"
                        // });
                        vm.getMenuList();
                        ...
                        vm.isLoginError = true;
                        vm.loginStatus = "Your account was accessed by a ban, please contact your administrator";
                        setTimeout(function () {
                          vm.isLoginError = false;
                          vm.loginStatus = "";
                        }, 2000);
      ...
      getMenuList() {
        ...
        request.http(
          ...
          success => {
            ...
            if (code === 200) {
              // console.log(success)
              localStorage.setItem("dove-cms-menus", JSON.stringify(success.data));
              localStorage.setItem("dove-cms-isLogin", 'true');
              vm.$router.push({
                path: "/home"
              });
              ...
              alert("Please contact the administrator role for the users to add");
            } else if (code === 306) {
              alert("Role has not yet been configuration menu");
            } else {
      ...
      setUserMenu(data) {
        ...
        localStorage.setItem("dove-menu", arr.join(","));
        localStorage.setItem("dove-cms-isLogin", 'true');
      }
    }
  };
