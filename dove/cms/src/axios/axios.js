...
export default {
  http (method, url, params, callback, fallback, headers) {
    var header = {
      authorization: localStorage.getItem('dove-cms-token')
    }
    // console.log(header)
    var o = {}
    if (method === 'get') {
  ...
  loginAgain (vm) {
    ...
    vm.$alert('token过期，请重新登录', 'Reminder', {
      confirmButtonText: '确定',
      callback: action => {
        localStorage.removeItem('dove-cms-token')
        localStorage.removeItem('dove-cms-loginname')
        localStorage.removeItem('dove-cms-menus')
        localStorage.removeItem('dove-cms-isLogin')
      }
    })
  },
  ...
  upload (method, url, params, callback, fallback, headers) {
    var header = {
      ...
      token: localStorage.getItem('dove-cms-token')
    }
    var o = {}
    o = {
      method: method,
