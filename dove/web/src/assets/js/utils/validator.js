 ...
 * 19段：180、181、182、183、184、185、186、187、188、189
 * @param {Object} value
 * @return {Boolean}
 */
...
export function idCard(value) {
  if (value) {
    ...
    if (regMatch != null) {
      ...
      if (length === 15) {
        a = new Date(
          "19" + regMatch[3] + "/" + regMatch[4] + "/" + regMatch[5]
        );
        r =
          a.getYear() == regMatch[3] &&
          a.getMonth() + 1 == regMatch[4] &&
          a.getDate() == regMatch[5];
      } else {
        a = new Date(regMatch[3] + "/" + regMatch[4] + "/" + regMatch[5]);
        r =
          a.getFullYear() == regMatch[3] &&
          a.getMonth() + 1 == regMatch[4] &&
          a.getDate() == regMatch[5];
      }
      return r;
 ...
 * @param {Object} value
 */
export function bankCard12(value) {
  if (value) {
...
// 3-16位字符，只能包含英文字母或数字，且必须同时包含数字和字母，
...
//6位以上字符，只能包含英文字母或数字，且必须同时包含数字和字母，不允许连续三位相同
