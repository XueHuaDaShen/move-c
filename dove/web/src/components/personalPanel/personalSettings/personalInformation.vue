  ...
  <div class="personalInformation" v-loading="loading" element-loading-background="rgba(0, 0, 0, 0.5)">
    <div class="personalInformation-inner">
      ...
      <div class="fill-amount">
        <div class="user-tab-line mt-30">{{$t('message.personal_information')}}</div>
        <div class="amount-row">
          <div class="flex-col">
            <div class="mt-30">{{$t('message.first_name')}} <span class="red">*</span></div>
          ...
          <div class="flex-col ml-30">
            <div class="mt-30">{{$t('message.last_name')}} <span class="red">*</span></div>
            <div class="mt-10 input-outer">
        ...
        <div class="amount-row">
          <div class="flex-col">
            <div class="mt-20">{{$t('message.bank_for_deposit_withdrawal')}} <span class="red">*</span></div>
            <div class="mt-10 input-outer">
        ...
        <div class="amount-row">
          <div class="flex-col">
            <div class="mt-20">{{$t('message.account_number')}} <span class="red">*</span></div>
            <div class="mt-10 input-outer">
        ...
        <div class="user-tab-line mt-30">{{$t('message.other_information')}}</div>
        <div class="amount-row">
          <div class="flex-col">
            <div class="mt-30">{{$t('message.phone_number')}} <span class="red">*</span></div>
            <div class="mt-10 input-outer">
        ...
        <div class="amount-row">
          <div class="flex-col">
            <div class="mt-20">{{$t('message.line_id')}}</div>
            <div class="mt-10 input-outer">
        ...
        <div class="amount-row">
          <div class="flex-col">
            <div class="mt-20">{{$t('message.email')}}</div>
            <div class="mt-10 input-outer">
        ...
        <div class="mt-60 amount-row pb-20">
          <button class="user-btn deposit-btn" @click="onSubmit()">{{$t('message.confirm')}}</button>
        </div>
      </div>
    </div>
  </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      // 更新个人资料
      updateProfile(data) {
        if(data.bankid === this.is12NumBankId){
          if (!validator.bankCard12(this.form.card_no.val)) {
            this.$store.state.message(this.$t('message.bank_format_is_incorrect'), 'warning', 3000, this);
            return false;
          }
        }else{
        ...
        if (!validator.cellphone(this.form.phone.val)) {
          this.$store.state.message(this.$t('message.phone_format_is_incorrect'), 'warning', 3000, this);
          return false;
        }
        if (!validator.email(this.form.email.val)) {
          this.$store.state.message(this.$t('message.email_format_is_incorrect'), 'warning', 3000, this);
          return false;
        }
        ...
        vm.$http.post(
          ...
          if (status) {
            if (status == 103 || status == 106 || status == 101) {
              ...
              vm.$store.state.message(this.$t('message.submit_success'), 'success', 3000, vm);
              for (let key in vm.form) {
                let obj = vm.form[key];
              ...
              // phone number 重复
              vm.$store.state.message(this.$t('message.phone_in_use'), 'warning', 3000, vm);
            } else if (status == 303) {
              // account number 重复
              vm.$store.state.message(this.$t('message.account_in_use'), 'warning', 3000, vm);
            } else {
              vm.$store.state.message(this.$t('message.submit_failure'), 'error', 3000, vm);
            }
          }
        }, error => {
          console.log('error', error);
