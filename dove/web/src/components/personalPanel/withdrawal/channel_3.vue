<template>
  <div class="withdrawal">
    ...
    <div class="withdrawal-inner">
      ...
      <div class="mt-30">
        <button class="user-btn withdrawal-btn" @click="onSubmit()">{{$t('message.confirm_withdrawal')}}</button>
      </div>
    </div>
  </div>
</template>
      ...
      onSubmit() {
        ...
        if (validate) {
          if(data.amount > max || data.amount < min){
            vm.$store.state.message(vm.$t('message.paytrust_withdrawal_limit'), 'warning', 3000, vm);
          }else{
            vm.withdrawal(data)
          }
        }
